...
static void expect_color_eq(t_color a, t_color b, char *flag, int i) {
	cr_expect(float_eq(a.r, b.r), "%s %i: expected r:%f, returned r:%f.", flag, i, b.r, a.r);
	cr_expect(float_eq(a.g, b.g), "%s %i: expected g:%f, returned g:%f.", flag, i, b.g, a.g); cr_expect(float_eq(a.b, b.b), "%s %i: expected b:%f, returned b:%f.", flag, i, b.b, a.b);}

static void expect_color_is_nan(t_color a,char *flag, int i) {
...
//static void assert_color_eq(t_color a, t_color b, char *flag, int i) {
...
Test(canvas, canvas_set_pixel) {
	...
	can = canvas_init(10, 10);
	color = color_set(0, 0, 0);
	ret = canvas_set_pixel(can, 20, 5, color);
	cr_expect(ret == 1, "%s %i: expected: %i, returned: %i\n", flag, i++, 1, ret);

	ret = canvas_set_pixel(can, 10, 5, color);
	cr_expect(ret == 1, "%s %i: expected: %i, returned: %i\n", flag, i++, 1, ret);

	ret = canvas_set_pixel(can, 5, 20, color);
	cr_expect(ret == 1, "%s %i: expected: %i, returned: %i\n", flag, i++, 1, ret);

	ret = canvas_set_pixel(can, 5, 10, color);
	cr_expect(ret == 1, "%s %i: expected: %i, returned: %i\n", flag, i++, 1, ret);

	can = canvas_init(10, 10);
	expect_color_eq(can->image[5][5], color, flag, i++);
	color = color_set(255, 0, 0);
	ret = canvas_set_pixel(can, 5, 5, color);
...
Test(canvas, canvas_get_pixel) {
	...
	can = canvas_init(10, 10);
	pixel = canvas_get_pixel(*can, 20, 5);
	expect_color_is_nan(pixel, flag, i++);

	pixel = canvas_get_pixel(*can, 10, 5);
	expect_color_is_nan(pixel, flag, i++);

	pixel = canvas_get_pixel(*can, 5, 20);
	expect_color_is_nan(pixel, flag, i++);

	pixel = canvas_get_pixel(*can, 5, 10);
	expect_color_is_nan(pixel, flag, i++);
